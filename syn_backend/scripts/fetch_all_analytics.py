...
Orchestrates data collection from Douyin, Kuaishou, Xiaohongshu, and Tencent Video
"""
import sys
...
from scripts.fetch_tencent_analytics import fetch_tencent_analytics
...
def fetch_analytics_for_account(db_path: Path, account_id: int, task_id: int = None, headless: bool = True):
    ...
    try:
        if platform == 'douyin':
            ...
            fetch_tencent_analytics(cookie, task_videos, db_path, account_id, headless)
        else:
