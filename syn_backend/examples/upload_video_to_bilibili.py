"""
B站批量上传示例（依赖 biliup-app/biliup 库）
---------------------------------------------------------------------------
- 先使用 `syn_backend/examples/get_bilibili_cookie.py`（内部调用 biliup.exe）扫码登录，
  同步 cookie 到 `syn_backend/uploader/bilibili_uploader/account.json`
- 然后运行本脚本，它会读取 account.json 并通过 biliup（python 库）上传视频
GitHub: https://github.com/Laihiujin/biliup-app
"""
