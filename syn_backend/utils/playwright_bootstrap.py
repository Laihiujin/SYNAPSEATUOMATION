...
def _chromium_glob_patterns(browsers_dir: Path) -> list[str]:
    if sys.platform == "win32":
        return [
            str(browsers_dir / "chromium-*" / "chrome-win" / "chrome.exe"),
            str(browsers_dir / "chromium-*" / "chrome-win64" / "chrome.exe"),
        ]
    if sys.platform == "darwin":
        return [str(browsers_dir / "chromium-*" / "chrome-mac" / "Chromium.app" / "Contents" / "MacOS" / "Chromium")]
    return [str(browsers_dir / "chromium-*" / "chrome-linux" / "chrome")]
...
def ensure_playwright_chromium_installed(
    *,
    ...
    # If user provided empty string (common in bat files), treat as unset.
    ...
    if chromium_exe and not expected_exe:
        return PlaywrightBootstrapResult(
            browsers_path=os.environ["PLAYWRIGHT_BROWSERS_PATH"],
