...
def _media_crawler_script() -> Path:
    return Path(settings.BASE_DIR) / "scripts" / "mediacrawler_collect.py"


def _mediacrawler_profile_id(platform: str) -> str:
    key = f"MEDIACRAWLER_{platform.upper()}_PROFILE"
    return (os.getenv(key) or _MEDIACRAWLER_PROFILE_IDS.get(platform) or f"mediacrawler_{platform}")
...
async def _run_mediacrawler_collect(
    platform: str,
    cookie_file: str,
    user_id: Optional[str],
    account_id: Optional[str],
) -> Dict[str, Any]:
    script_path = _media_crawler_script()
    if not script_path.exists():
        return {"success": False, "error": "mediacrawler script missing"}
    ...
    env = os.environ.copy()
    chrome_path = Path(settings.BASE_DIR) / "browsers" / "chromium" / "chromium-1161" / "chrome-win" / "chrome.exe"
    ...
    try:
        return json.loads(output) if output else {"success": False, "error": "no output"}
    except Exception:
        return {"success": False, "error": output or "invalid output"}
...
async def _collect_xhs_mediacrawler_accounts(account_ids: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    profile_account_id = _mediacrawler_profile_id("xiaohongshu")
    profile_cookie_file = _ensure_mediacrawler_cookie_file("xiaohongshu")
    ...
    for account in accounts:
        ...
        cookie_file = account.get("cookie_file") or ""
        cookie_data = cookie_manager._read_cookie_file(cookie_file)
        user_id = _extract_xhs_user_id(cookie_data) or account.get("user_id")
        result = await _run_mediacrawler_collect(
            "xiaohongshu",
            profile_cookie_file,
            user_id,
            profile_account_id,
        ...
        if not result.get("success"):
            failed.append({"account_id": account_id, "error": result.get("error") or "collect_failed"})
            continue

        items = result.get("items") or []
        for note in items:
            record = _normalize_xhs_note(account_id, note)
            if record:
                upsert_video_analytics_by_key(DB_PATH, platform="xiaohongshu", video_id=record["video_id"], data=record)
        success += 1
...
async def _collect_kuaishou_mediacrawler_accounts(account_ids: Optional[List[str]] = None) -> Dict[str, Any]:
    accounts = [
        acc for acc in cookie_manager.list_flat_accounts()
        if acc.get("platform") == "kuaishou"
    ]
    if account_ids:
        accounts = [acc for acc in accounts if acc.get("account_id") in set(account_ids)]

    profile_account_id = _mediacrawler_profile_id("kuaishou")
    profile_cookie_file = _ensure_mediacrawler_cookie_file("kuaishou")
    ...
    for account in accounts:
        ...
        items = result.get("items") or []
        for item in items:
            record = _normalize_kuaishou_item(account_id, item)
            if record:
                upsert_video_analytics_by_key(DB_PATH, platform="kuaishou", video_id=record["video_id"], data=record)
        success += 1
...
async def get_analytics(
    ...
    """获取汇总、视频列表和图表数据"""
    try:
        summary = get_analytics_summary(DB_PATH, startDate, endDate, platform)
