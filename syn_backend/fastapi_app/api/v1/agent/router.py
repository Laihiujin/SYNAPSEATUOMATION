    ...
    try:
        ...
        async with _manus_stream_lock:
            ...
            while manus.current_step < max_steps and manus.state != AgentState.FINISHED:
                ...
                # 尝试把 agent 的“本轮输出/思考”也流式传给前端（用于对话区展示）
                try:
                ...
                # 没有工具调用：如果 should_act 为 True，act() 可能会返回纯文本回答
                if should_act:
                    stream_step += 1
                    act_result = await manus.act()
                    yield _sse({"type": "step_complete", "step": stream_step, "result": act_result})
                    continue
            ...
            try:
                last_assistant = next(
                    (m.content for m in reversed(getattr(manus.memory, "messages", [])) if getattr(m, "role", "") == "assistant" and getattr(m, "content", None)),
                    "",
                )
            except Exception:
                last_assistant = ""
