...
class OpenAICompatibleProvider(BaseProvider):
    ...
    def _init_client(self):
        try:
            ...
            print(f"[OpenAI Compatible] Initializing with Base URL: {processed_base_url}...")
            self.client = OpenAI(api_key=self.api_key, base_url=processed_base_url)
        except ImportError:
            print("[OpenAI Compatible] OpenAI SDK not installed")
            self.client = None
        except Exception as e:
            print(f"[OpenAI Compatible] Error initializing client: {e}")
            self.client = None
    ...
    async def get_available_models(self, model_type: Optional[str] = None, sub_type: Optional[str] = None) -> List[AIModel]:
        ...
        try:
            ...
            print(f"[OpenAI Compatible] Fetching all models (ignoring filters: type={model_type}, sub_type={sub_type})")
            ...
            # 使用 built-in type 函数
            import builtins
            print(f"[OpenAI Compatible] Raw response type: {builtins.type(response)}")
            print(f"[OpenAI Compatible] Raw response data: {response}")
            ...
            print(f"[OpenAI Compatible] Found {len(data_list)} models in response")
            ...
            print(f"[OpenAI Compatible] Failed to list models: {e}")
            return []
