...
class ModelManager:
    ...
    def _load_config(self):
        ...
        try:
            ...
            # Resolve DB path relative to this repo by default; allow override via env.
            base_dir = Path(__file__).resolve().parent.parent  # syn_backend
            db_path = os.getenv("SYNAPSE_DATABASE_PATH") or str(base_dir / "db" / "database.db")
    ...
    def _init_provider(self, provider_name: str, api_key: str, base_url: Optional[str] = None):
        ...
        try:
            if provider_name == "siliconflow":
                self.providers[provider_name] = SiliconFlowProvider(api_key)
            elif provider_name == "volcanoengine":
                self.providers[provider_name] = VolcanoEngineProvider(api_key)
            elif provider_name == "tongyi":
                self.providers[provider_name] = TongyiProvider(api_key)
            elif provider_name == "openai_compatible":
                if not base_url:
                    # 尝试从配置中获取默认值，或者报错
                    # 这里假设调用者必须提供 base_url
                    print(f"Warning: base_url is required for {provider_name}")
                    return
                self.providers[provider_name] = OpenAICompatibleProvider(api_key, base_url)
        except Exception as e:
