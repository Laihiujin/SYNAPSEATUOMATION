...
export default function PlatformVideosPage() {
    ...
    const sortedVideos = useMemo(() => {
        return [...videos].sort((a: any, b: any) => (b.views || 0) - (a.views || 0))
    }, [videos])
    ...
    return (
        <div className="space-y-8 px-4 py-4 md:px-6 md:py-6 animate-in fade-in duration-500">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-2xl font-bold text-white">{config.label} 矩阵概览</h2>
                        </div>
            ...
            <div className="grid gap-6 lg:grid-cols-4">
                {/* Video List */}
                <Card className="lg:col-span-2 bg-black/40 border-black overflow-hidden flex flex-col">
                    <CardHeader className="border-b border-black/20 bg-black/20">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription className="text-white/40">按权重计算的表现最佳内容</CardDescription>
                            </div>
                            <Select defaultValue="views">
                                <SelectTrigger className="w-32 bg-black/60 border-black h-8 text-xs rounded-lg">
                                    <SelectValue placeholder="排序方式" />
                                </SelectTrigger>
                                <SelectContent className="bg-black border-black text-white">
                        ...
                        <div className="divide-y divide-black/20">
                            {isLoading ? (
                                ...
                                sortedVideos.map((video: any, idx: number) => (
                                    <div key={idx} className="group flex items-center gap-3 p-3 hover:bg-white/5 transition-colors">
                                        <div className="relative shrink-0">
                                            <img
                                                src={video.cover_url || "/placeholder-video.png"}
                                                className="w-20 h-28 object-cover rounded-xl border border-black"
                                                alt=""
                                            />
                                            <div className="absolute top-1 left-1 bg-black/60 backdrop-blur-md px-1.5 py-0.5 rounded-lg text-[10px] text-white/80 font-bold">
                                                #{idx + 1}
                                            </div>
                                        </div>
                                        <div className="flex-1 min-w-0 flex flex-col justify-between py-1 h-24">
                                            <div>
                                                <h4 className="text-white font-medium line-clamp-1 leading-snug group-hover:text-primary transition-colors">
                                                    {video.title || "未命名视频"}
                                                </h4>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[10px] text-white/40 bg-black/40 px-2 py-0.5 rounded-full border border-black uppercase tracking-wider">
                                            ...
                                            <div className="flex items-center justify-between mt-auto">
                                                <div className="flex items-center gap-3">
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] text-white/30">播放</span>
                                                        <span className="text-sm font-bold text-white">{formatNumber(video.views)}</span>
                                                    </div>
                                                    <div className="flex flex-col border-l border-black pl-4">
                                                        <span className="text-[10px] text-white/30">互动</span>
                                                        <span className="text-sm font-bold text-white">{formatNumber(video.likes + video.comments)}</span>
                                                    </div>
                                                </div>

                                                <Button size="sm" variant="ghost" className="h-8 w-8 rounded-full hover:bg-white/10">
                ...
                <div className="space-y-6 lg:col-span-2">
                    <Card className="bg-black/40 border-black overflow-hidden relative">
                        <CardHeader>
                            <CardTitle className="text-lg text-white">平台指标分布</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="space-y-6">
                                {config.metrics.map((metric, i) => {
                                    const value = (stats as any)[metric] || 0
                                    const percent = stats.views > 0 ? Math.min(100, Math.round((value / stats.views) * 100 * (i + 1))) : 20 * (5 - i)
                                    return (
                                        <div key={metric} className="space-y-2">
                                            <div className="flex justify-between text-xs">
                                                <span className="text-white/60 capitalize">{metric}</span>
                    ...
                    <Card className="bg-gradient-to-br from-gray-900 to-black border-black relative overflow-hidden group">
                        <CardContent className="p-6 flex flex-col items-center text-center">
                            <div className="p-4 bg-white/5 rounded-3xl mb-4 group-hover:scale-110 transition-transform">
                                <TrendingUp className="h-8 w-8 text-primary" />
                            </div>
                            <h3 className="text-white font-bold mb-2">自动化策略建议</h3>
                            <p className="text-xs text-white/60 mb-6 px-4">
                                根据目前的 {config.label} 数据表现，系统建议您增加晚间 8-10 点的发布权重，以提升互动转化。
                            </p>
