...
export function CreateCampaignDialog({ open, onOpenChange, onSuccess }: CreateCampaignDialogProps) {
    ...
    const fetchMaterials = async () => {
        try {
            ...
            setMaterials(items.filter((m: any) => m.type?.startsWith('video') || m.filename?.endsWith('.mp4')))
        } catch (e) {
    ...
    const handleCreate = async () => {
        ...
        if (selectedPlatforms.length === 0) return toast({ variant: "destructive", title: "请选择至少一个平台" })
        if (selectedAccounts.length === 0) return toast({ variant: "destructive", title: "请选择至少一个账号" })
        if (selectedMaterials.length === 0) return toast({ variant: "destructive", title: "请选择至少一个素材" })
        ...
        try {
            ...
            if (data.status === "success") {
                toast({ title: `计划创建成功，生成 ${data.result.tasks_created} 条任务` })
                onSuccess()
                onOpenChange(false)
            } else {
            ...
            console.error(e)
            toast({ variant: "destructive", title: "创建请求失败" })
        } finally {
            setSubmitting(false)
        }
    }
                        ...
                        <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {materials.map(m => (
                                            <div
                                                key={m.id}
                                                className={cn(
                                                    "relative aspect-video rounded-lg border overflow-hidden cursor-pointer group",
                                                    selectedMaterials.includes(m.id) ? "ring-2 ring-primary border-primary" : "hover:border-primary/50"
                                                )}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div
                                        className={cn("border rounded-lg p-4 cursor-pointer text-center hover:bg-muted", scheduleType === 'immediate' && "border-primary bg-primary/5")}
                                    ...
                                    <div
                                        className={cn("border rounded-lg p-4 cursor-pointer text-center hover:bg-muted", scheduleType === 'range' && "border-primary bg-primary/5")}
                                    ...
                                    <div
                                        className={cn("border rounded-lg p-4 cursor-pointer text-center hover:bg-muted", scheduleType === 'daily' && "border-primary bg-primary/5")}
                            ...
                            <div className="border rounded-md">
                                ...
                                <ScrollArea className="h-[300px]">
                                    {previewTasks.map((task, i) => (
                                        <div key={i} className="px-4 py-3 text-sm grid grid-cols-5 gap-4 border-b last:border-0 hover:bg-muted/50">
                                            <div className="flex items-center gap-2">
